...
class ModelHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        ...
        if is_vol:
            ...
            vol3 = [self.bsm_model.impvol(price_or_vol, strike, spot, texp, cp_sign=cp_sign)
                        for (price_or_vol, strike) in zip(price_or_vol3, strike3)]
        def FOC(x):
            ...
            bsm_vols = [bsm_vol(strike, spot, texp, sigma, alpha, rho) - vol 
                            for (strike, vol) in zip(strike3, vol3)]
        ...
        alpha = np.exp(solution_x[1])
        rho = 2 / (1 + np.exp(-solution_x[2])) - 1
...
class ModelNormalHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        ...
        if is_vol:
            ...
            vol3 = [self.normal_model.impvol(price_or_vol, strike, spot, texp, cp_sign=cp_sign)
                        for (price_or_vol, strike) in zip(price_or_vol3, strike3)]
        def FOC(x):
            ...
            norm_vols = [norm_vol(strike, spot, texp, sigma, alpha, rho) - vol 
                            for (strike, vol) in zip(strike3, vol3)]
...
class ModelBsmMC:
    ...
    def bsm_vol(self, strike, spot, texp=None, sigma=None, setval=False):
        ...
        texp = self.texp if(texp is None) else texp
        vol = self.bsm_model.impvol(price, strike, spot, texp, cp_sign=1)
        forward = spot * np.exp(texp*(self.intr - self.divr))
...
class ModelNormalMC:
    ...
    def norm_vol(self, strike, spot, texp=None, sigma=None, setval=False):
        ''''
        From the price from self.price() compute the implied vol
        ...
        texp = self.texp if(texp is None) else texp
        vol = self.normal_model.impvol(price, strike, spot, texp, cp_sign=1)
        forward = spot * np.exp(texp*(self.intr - self.divr))
...
class ModelBsmCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        texp = texp if texp else self.texp

        delta_t = self.texp / self.step

        z = np.random.normal(size = (strike.size, self.step, self.n_samples))

        sigma_ = np.exp(-0.5* self.alpha**2 * delta_t + self.alpha * np.sqrt(delta_t) * z)
        sigma_path = self.sigma * np.cumprod(sigma_, axis=1)
        I = np.sum(sigma_path ** 2 * delta_t, axis=1)
        new_spot = spot*np.exp(self.rho * (sigma_path[:, -1] -  self.sigma) / self.alpha - self.rho ** 2 / 2 * I)
        new_sigma = np.sqrt((1-self.rho ** 2) * I / texp)
        price = [[self.bsm_model.price(strike[i], spot, texp, sigma, cp_sign)
            for (spot, sigma) in zip(new_spot[i,:], new_sigma[i, :])] for i in np.arange(strike.size)]
...
class ModelNormalCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        sigma_ = np.exp(-0.5* self.alpha**2 * delta_t + self.alpha * np.sqrt(delta_t) * z)
        sigma_path = sigma * np.cumprod(sigma_, axis=1)
        I = np.sum(sigma_path ** 2 * delta_t, axis=1)
        new_spot = spot + self.rho * (sigma_path[:,-1] -  sigma) / self.alpha
        new_sigma = np.sqrt((1-self.rho ** 2) * I / texp)
        price = [[self.normal_model.price(strike[i], spot, texp, sigma, cp_sign)
            for (spot, sigma) in zip(new_spot[i,:], new_sigma[i, :])] for i in np.arange(strike.size)]
