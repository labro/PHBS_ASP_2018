...
def basket_price_mc(
    ...
    if bsm:
        ...
        '''
        cov_m = vol * cor_m * vol[:, np.newaxis]
        chol_m = np.linalg.cholesky(cov_m)
        prices = forward[:, np.newaxis] * \
            np.exp(
            (-0.5 * texp * vol * vol).reshape((vol.shape[0], 1))
...
def spread_price_kirk(
    strike, spot, vol, texp, corr, intr=0, divr=0, cp_sign=1
):
    div_fac = np.exp(-texp*divr)
    disc_fac = np.exp(-texp*intr)
    forward = spot / disc_fac * div_fac
    ...
    price = disc_fac * \
        bsm_price(forward[1]+strike, forward[0], vol_r, texp, cp_sign=cp_sign)
